...
- Once a creature is Apex, its **sheet reads as a solo boss at a glance**: a signal-amber frame on the window, a glowing crown in the title bar, and an etched **APEX** emblem banner (with the turns-per-round count) at the top of the sheet body. Toggling Apex off clears the treatment on the next render.
...
- **Condition-counteract action (Component 3).** A **shield button** on the boss's Prime row (and `api.apexCounteract(actor)`) rolls the once-per-turn counteract using the boss's **highest save modifier** at its **full creature level**, and posts it for you to compare against the condition's DC.
